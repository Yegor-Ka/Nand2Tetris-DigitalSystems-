...
public class Main {
    ...
    private static void processJackFile(File jackFile) {
        try {
            ...
            File outputFile = new File(basePath + ".xml");
            File tokenFile = new File(basePath + "T.xml");

            // Initialize the CompilationEngine and compile the class
            CompilationEngine compiler = new CompilationEngine(jackFile, outputFile, tokenFile);
            compiler.compileClass();

            // Output success messages
            System.out.println("Compiled File: " + outputFile.getAbsolutePath());
            System.out.println("Token File: " + tokenFile.getAbsolutePath());
        } catch (Exception e) {
